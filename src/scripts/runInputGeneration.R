...
for (numberOfProviders in minNumberOfProviders:maxNumberOfProviders) {
  ...
  while (turn <= numberOfTurns) {
    ...
    # 01 - AS POLITICAS DE DISPONIBILIDADE E FIDELIDADE PODEM SER IMPLEMENTADAS NO ARQUIVO .TXT ANTES DA OTIMIZACAO
    # 02 - UMA HEURISTICA ALEATORIA PODE SER IMPLEMENTADA A PARTIR DE TODAS OS RECURSOS VALIDOS
    # (SELECIONA ALEATORIAMENTE PARA CADA DEMANDA E REMOVE AS DEMAIS OFERTA PARA TAL DEMANA)
    # (ALEM DE REMOVER AS DEMAIS ENTRADAS COM O MESMO CONJUNTO <PROVEDOR,RECURSO>)

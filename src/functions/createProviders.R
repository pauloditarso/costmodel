createProviders <- function(numberOfProviders, numberOfHosts = 0, numberOfLinks = 0, numberOfNEs = 0, minHosts, minLinks, minNEs) {
	...
	if (numberOfProviders > 0) {
	  ...
	  # if ( missing(numberOfNEs) ) { numberOfNEs <- sample(0:3, size=1) }
		...
		for (i in 1:numberOfProviders) {
			name <- paste("p", i, sep = "")
			assign(name, createOneProvider(numberOfHosts, numberOfLinks, numberOfNEs, minHosts, minLinks, minNEs), envir = auxEnv)
		}
